...
CREATE TABLE IF NOT EXISTS `site_docs` (
  `site_docs_id` INT NOT NULL,
  `site_docs_name` VARCHAR(45) NULL,
  `site_docs_path` VARCHAR(45) NULL,
  `site_docs_type` ENUM("Background reading", "Schematic", "Safety protocol", "Methodology", "Draft report", "Final report") NULL,
  PRIMARY KEY (`site_docs_id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `permissions` (
  ...
  `permissions_level` ENUM("Read", "Read/Write", "read/Write/Delete", "None") NULL,
  `permissions_role` ENUM("Site owner", "Site manager", "Site worker", "Site recorder") NULL,
  `user_id` INT NULL,
  `site_docs_id` INT NOT NULL,
  PRIMARY KEY (`permissions_id`),
  INDEX `fk_permissions_user1_idx` (`user_id` ASC) VISIBLE,
  INDEX `fk_permissions_site_docs1_idx` (`site_docs_id` ASC) VISIBLE,
  CONSTRAINT `fk_permissions_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `user` (`user_id`)
    ON DELETE RESTRICT
    ON UPDATE CASCADE,
  CONSTRAINT `fk_permissions_site_docs1`
    FOREIGN KEY (`site_docs_id`)
    REFERENCES `site_docs` (`site_docs_id`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `continuation` (
  `continuation_id` INT NOT NULL AUTO_INCREMENT,
  `continuation_type` ENUM("Unit record", "Photo", "Sketch", "Map", "Artifact record") NULL,
  `continuation_path` VARCHAR(45) NULL,
  `site_record_id` SMALLINT(8) NOT NULL,
  PRIMARY KEY (`continuation_id`),
  INDEX `fk_continuation_site_record1_idx` (`site_record_id` ASC) VISIBLE,
  CONSTRAINT `fk_continuation_site_record1`
    FOREIGN KEY (`site_record_id`)
    REFERENCES `site_record` (`site_record_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
...
-- -----------------------------------------------------
-- Table `site_record_has_site_docs`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `site_record_has_site_docs` ;

CREATE TABLE IF NOT EXISTS `site_record_has_site_docs` (
  `site_record_id` SMALLINT(8) NOT NULL,
  `site_docs_id` INT NOT NULL,
  PRIMARY KEY (`site_record_id`, `site_docs_id`),
  INDEX `fk_site_record_has_site_docs_site_docs1_idx` (`site_docs_id` ASC) VISIBLE,
  INDEX `fk_site_record_has_site_docs_site_record1_idx` (`site_record_id` ASC) VISIBLE,
  CONSTRAINT `fk_site_record_has_site_docs_site_record1`
    FOREIGN KEY (`site_record_id`)
    REFERENCES `site_record` (`site_record_id`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE,
  CONSTRAINT `fk_site_record_has_site_docs_site_docs1`
    FOREIGN KEY (`site_docs_id`)
    REFERENCES `site_docs` (`site_docs_id`)
    ON DELETE NO ACTION
    ON UPDATE CASCADE)
